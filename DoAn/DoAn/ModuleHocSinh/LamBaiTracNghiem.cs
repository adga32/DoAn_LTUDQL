...
namespace DoAn
{
    public partial class LamBaiTracNghiem : MetroFramework.Forms.MetroForm
    {
        ...
        //loaiBaiThi là biến kiểm tra người dùng ôn tập hay thi thật
        ...
        private void btnNopBai_Click(object sender, EventArgs e)
        {
            ...
            using (var db = new DoAnDataContext())
            {
                string ID_NguoiDung = db.taiKhoans.Where(tk => tk.username == nguoiDungHienTaiLamBai).Select(tk => tk.idNguoiSoHuu).Single();
                ...
                for (int i = 0; i < danhSachCauHoi.Count; i++)
                {
                    ...
                    // kiểm tra nếu là đang thi thật thì lưu thông tin chi tiết câu trả lời từng câu hỏi
                    if (laKiemTraChinhThuc)
                    {
                        db.chiTietBaiThis.InsertOnSubmit(new chiTietBaiThi
                        {
                            maHs = ID_NguoiDung,
                            maDe = maDeThi,
                            maKythi = maKyThi,
                            maCauHoi = danhSachCauHoi[i].maCauHoi,
                            dung_Sai = trueOrFalseInsert
                ...
                if (laKiemTraChinhThuc)
                {
                    luuKetQuaBaiLam(soCauDung, Diem);
                }

                MessageBox.Show("đúng: " + soCauDung + "/" + TongCau + "\nĐiểm: " + Diem);
                return;
